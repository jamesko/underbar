...
(function() {
  ...
  _.reject = function(collection, test) {
    ...
    var invertedTest = function(item){return !(test(item))};
  ...
  _.every = function(collection, iterator) {
    ...
    if(arguments[1] == undefined) {iterator = _.identity};
    return _.reduce(collection, function(allPriorTests, item){
      if([null, 0, undefined].indexOf(iterator(item))>-1) return false;
      return iterator(item) && allPriorTests;
    }, true);
  };
  ...
  _.shuffle = function(array) {
    ...
    while(copyArray.length>0){
      pickdex = Math.floor(Math.random()*copyArray.length);
      shuffledArray.push(copyArray.splice(pickdex,1)[0])
    };
